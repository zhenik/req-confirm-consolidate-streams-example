...
public class StreamJoinApplication implements Runnable {
  ...
  private final String topicResult = "results-v1";

  public StreamJoinApplication() { this.kafkaStreams = new KafkaStreams(buildTopology(), getDefault()); }
  private Topology buildTopology() {
    ...
    final KStream<String, String> consolidatedStream = requestsStream.join(
        ...
        (requestValue, confirmedValue) -> confirmedValue,
        // window
    ...
    consolidatedStream
        ...
        .to(topicResult, Produced.with(Serdes.String(), Serdes.String()));
  ...
  @Override public void run() {
    Util.createTopic(topicRequest);
    Util.createTopic(topicConfirmation);
    Util.createTopic(topicResult);
    kafkaStreams.start();
  }
